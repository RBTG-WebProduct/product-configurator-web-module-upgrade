...
class ProductConfigWebsiteSale(WebsiteSale):
    ...
    def get_render_vals(self, cfg_session):
        ...
        product_configurator_obj = request.env["product.configurator"].sudo()
        ...
        check_val_ids = (
            cfg_session.product_tmpl_id.sudo().attribute_line_ids.mapped("value_ids")
            + custom_val_id
        )
        ...
        product_tmpl = cfg_session.product_tmpl_id.sudo()
        attr_value_ids = product_tmpl.attribute_line_ids.mapped("value_ids")
        av_obj = request.env["product.attribute.value"].sudo()
        extra_prices = av_obj.get_attribute_value_extra_prices(
    ...
    def get_current_configuration(self, form_values, cfg_session):
        ...
        product_attribute_lines = product_tmpl_id.sudo().attribute_line_ids
        value_ids = []
    ...
    def get_orm_form_vals(self, form_vals, config_session):
        ...
        product_tmpl_id = config_session.product_tmpl_id.sudo()
        values = {}
    ...
    def get_config_product_template(self, form_vals):
        ...
        product_template_id = request.env["product.template"].sudo()
    ...
    def get_extra_attribute_line_ids(self, product_template_id):
        ...
        product_template_id = product_template_id.sudo()
    ...
    def onchange(self, form_values, field_name, **post):
        ...
        product_template_id = self.get_config_product_template(form_values).sudo()
        try:
    ...
    def save_configuration(
        ...
        try:
            ...
            if product:
                ...
                redirect_url += "/%s" % (
                    request.env["ir.http"]._slug(config_session_id)
                )
                return {
    ...
    def cfg_session(self, cfg_session_id, **post):
        ...
        reconfigure_product_url = "/product_configurator/reconfigure/%s" % request.env[
            "ir.http"
        ]._slug(product_id)
        values = {
